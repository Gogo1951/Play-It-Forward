...
--[[
	One-handers split two ways rather than one. Swords, maces and axes go to warriors;
	daggers go to rogues. Every class in the game can be handed a dagger and a rogue is
	the only one who builds around them, so pooling both weapon groups under "warriors
	and rogues" left the two competing for the same items when they each have their own.
]]
test("a one-hand sword is a warrior's", function()
	local ns = load()
	local verdict = ns.Matcher:Verdict(weapon(ns, SWORD, ONE_HAND, {}))

	equal(sorted(verdict.contenders), "WARRIOR", "the warrior leads")
	check(contains(verdict.admitted, "ROGUE"), "the rogue is behind him, not excluded")
end)
...
test("a one-hand mace stops going to druids first", function()
	...
	equal(sorted(verdict.contenders), "WARRIOR", "the warrior leads")
	check(contains(verdict.admitted, "DRUID"), "and the druid is still behind him, not excluded")
end)
...
--[[
	Fist weapons were not named either way. They sit with the warrior group because they
	are 1H melee like the rest of it, and a rogue is still a fallback for them -- worth
	knowing, since a combat rogue wants a fist weapon about as much as a warrior does.
]]
test("a fist weapon goes with the warrior group", function()
	local ns = load()
	local verdict = ns.Matcher:Verdict(weapon(ns, FIST, ONE_HAND, {}))

	equal(sorted(verdict.contenders), "WARRIOR", "the warrior leads")
	check(contains(verdict.admitted, "ROGUE"), "with the rogue behind him")
end)

test("a two-hander is for druids and paladins", function()
	local ns = load()
	local verdict = ns.Matcher:Verdict(weapon(ns, MACE_2H, TWO_HAND, {}))

	equal(sorted(verdict.contenders), "DRUID PALADIN", "the two the one-hand rule leaves out")
...
--[[
	Features/Matching-Engine.lua warns about this above ns.Data.UsesWeaponMatrix:
